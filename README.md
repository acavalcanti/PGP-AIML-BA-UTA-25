# PGP-AIML-BA-UTA-25

This is my Academic e-Portfolio containing all projects I have done during the Post Graduate Program in Artificial Intelligence and Machine Learning: Business Applications course by the McCombs School of Business at The University of Texas at Austin.

This was a 8 months course (May 2025 - December 2025)

Here is the public link with the overview of all module and projects:
https://www.mygreatlearning.com/eportfolio/jose-alexandre-tenorio-cavalcanti-junior

Course Modules:
1) Python Foundations (https://github.com/acavalcanti/PGP-AIML-BA-UTA-25/blob/main/Python/FoodHub.md)
   - Introduction to Python
   - Data Manipulation
   - Exploratory Data Analysis
   - Analyzing Text Data
2) Machine Learning (https://github.com/acavalcanti/PGP-AIML-BA-UTA-25/blob/main/ML/PersonalLoan.md)
   - Linear Regression
   - Decision Trees
   - K-means Clustering
3) Advanced Machine Learning (https://github.com/acavalcanti/PGP-AIML-BA-UTA-25/blob/main/AML/EasyVisa.md)
   - Bagging
   - Boosting
   - Model Tuning
4) Introduction To Neural Networks (https://github.com/acavalcanti/PGP-AIML-BA-UTA-25/blob/main/NN/ReneWind.md)
   - Introduction to Neural Networks
   - Optimizing Neural Networks
5) Natural Language Processing with Generative AI (https://github.com/acavalcanti/PGP-AIML-BA-UTA-25/blob/main/NLP/MedicalAssistant.md)
   - Word Embeddings
   - Attention Mechanism and Transformers
   - Large Language Models and Prompt Engineering
   - Retrieval Augmented Generation
6) Introduction to Computer Vision (https://github.com/acavalcanti/PGP-AIML-BA-UTA-25/blob/main/CV/HelmNet.md)
   - Image Processing
   - Convolutional Neural Networks
7) Model Deployment
   - Introduction to Model Deployment
   - Containerization
